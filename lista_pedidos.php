...
<head>
    <title>Listar pedidos Venda</title>
    <style>
        .container {
            padding-top: 20px;
        }
...
<body>
    ...
    <div class="container">
        <h3 style="margin-bottom:30px">Pedido de Venda</h3>
        <h6><b>Clique no Cod_pedido pra gerar comprovante, clique no Cliente pra abrir os detalhes do Recebimento.
                Destino é inserir o valor total no banco para controle.</h6>
        </b>
        <form method="GET" action="" style="margin-bottom: 20px;">
            <div class="input-group">
                <input type="text" class="form-control"
                    placeholder="Pesquisar por nome do cliente e cod_pedido ou status" name="pesquisar"
        ...
        $sql = "SELECT codigo_pedido, nome_cliente, responsavel_pedido, observacoes, valor_total, data, pago FROM `pedidos` WHERE (codigo_pedido LIKE '%$pesquisar%' OR nome_cliente LIKE '%$pesquisar%' OR pago LIKE '%$pesquisar%') AND deletado = 'N' LIMIT $offset, $registrosPorPagina";

        $retorno = mysqli_query($conexao, $sql);

        $totalRegistros = mysqli_num_rows(mysqli_query($conexao, "SELECT codigo_pedido FROM `pedidos` WHERE (nome_cliente LIKE '%$pesquisar%' OR pago LIKE '%$pesquisar%') AND deletado = 'N'"));
        ...
        <table class="table">
            ...
            <tbody>
                ...
                while ($array = mysqli_fetch_array($retorno, MYSQLI_ASSOC)) {
                    ...
                    <tr>
                        <td>
                            <a href="detalhes_pedido.php?codigo_pedido=<?= $codigo_pedido ?>" target="blank">
                                <?= $codigo_pedido ?>
                            </a>
                        </td>
                        <td>
                            <a href="info_recebimento.php?codigo_pedido=<?= $codigo_pedido ?>">
                                <?= $nome_cliente ?>
                            </a>
                        </td>
                        <td>
                        ...
                        <td>
                            <a class="btn-editar btn btn-sm btn-warning"
                                href="pedido_recebido.php?codigo_pedido=<?= $codigo_pedido ?>"
                                onclick="return confirm('Tem certeza de que deseja receber este pedido?')" role="button"><i
                                    class="fa fa-money"></i> Rec</a>
                            <a class="btn-editar btn btn-sm btn-info"
                                href="escolher_destino.php?codigo_pedido=<?= $codigo_pedido ?>&valor_total=<?= $valor_total ?>"
                                role="button">
                                <i class="fa fa-location-arrow"></i> Dest
                            </a>
                            <a class="btn-editar btn btn-sm btn-warning"
                                href="estornar_pedido.php?codigo_pedido=<?= $codigo_pedido ?>"
                            ...
                            <?php if ($nivel == 1) { ?>
                                <a class="btn-editar btn btn-sm btn-danger"
                                    href="excluir_pedido_venda.php?codigo_pedido=<?= $codigo_pedido ?>"
        ...
        <?php
        include_once 'conexao.php';
        $sql2 = "SELECT SUM(valor_total) AS total FROM pedidos 
         WHERE (codigo_pedido LIKE '%$pesquisar%' 
                OR nome_cliente LIKE '%$pesquisar%' 
        ...
        if ($retorno2) {
            ...
            echo "<b> Pedidos recebidos: <font color='green'>R$" . $total1 . "</font></b>";
        ...
        if ($retorno2) {
            ...
            // Exibe o resultado na tela
            echo "<b> Não recebidos: <font color='green'>R$" . $total2 . "</font></b> <br>";
        ...
        echo "<b> Total: <font color='green'>R$" . $total_valor . "</font></b> <br>";
    ...
    if (isset($_GET['recebido'])) {
        echo '<div id="alerta" class="alert alert-success" role="alert">
                Pedido <b>' . $_GET['recebido'] . '</b> recebido com sucesso!.
                </div>';
    }
