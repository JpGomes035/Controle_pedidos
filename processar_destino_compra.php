        ...
        if (isset($_POST['codigo_pedido']) && isset($_POST['destino']) && isset($_POST['valor_total'])) {
            ...
            if ($result->num_rows > 0) {
                ...
                if ($conexao->query($sql_update) === TRUE) {
                    // Aqui você adiciona o comando para marcar o pedido_compra como pago
                    $sql_update_pedido_compra = "UPDATE pedido_compra SET `pago` = 'S' WHERE `codigo_pedido` = $codigo_pedido";
                    if ($conexao->query($sql_update_pedido_compra) === TRUE) {
                        echo "Valor total do pedido retirado ao valor do banco '$destino' com sucesso. Pedido de compra marcado como pago.";
                    } else {
                        echo "Erro ao marcar o pedido de compra como pago: " . $conexao->error;
                    }
                } else {
